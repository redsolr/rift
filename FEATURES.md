...
- ✅ **Mobile (2026-08-17)**: ≤900px switches to a phone layout — HUD wraps into two rows (mode tabs + turn pill / playback + actions), the side panel becomes a draggable bottom sheet (peek → half → full; tap the handle to cycle) that sits *in flow* so the board shrinks and the camera re-fits (map always fully visible); one-finger pan, two-finger pinch/rotate; 40px touch targets; 16px inputs (no iOS zoom); safe-area insets; `apple-mobile-web-app-capable` for add-to-home-screen. Sheet handle shows a contextual hint ("Your move — tap a red unit", "Command phase — set orders, then Execute", result).
